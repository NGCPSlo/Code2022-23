			...
			while(True):
				...
				if (count >= BUFFER_SIZE):
					...
					if (avg_aspect <= ASPECT_UPPER) or (avg_aspect >= ASPECT_LOWER):
						...
						topLeft = dai.Point2f(average_box["Box"][0][0], average_box["Box"][0][1])
						bottomRight = dai.Point2f(average_box["Box"][3][0], average_box["Box"][3][1])
						...
						depthFrameColor = cv.normalize(depthFrame, None, 255, 0, cv.NORM_INF, cv.CV_8UC1)
						depthFrameColor = cv.equalizeHist(depthFrameColor)
						depthFrameColor = cv.applyColorMap(depthFrameColor, cv.COLORMAP_HOT)
						...
						for depthData in spatialData:
							...
							cv.rectangle(depthFrameColor, (xmin, ymin), (xmax, ymax), (255, 255, 255), cv.FONT_HERSHEY_SCRIPT_SIMPLEX)
							cv.putText(depthFrameColor, f"X: {int(depthData.spatialCoordinates.x)} mm", (xmin + 10, ymin + 20), fontType, 0.5, 255)
							cv.putText(depthFrameColor, f"Y: {int(depthData.spatialCoordinates.x)} mm", (xmin + 10, ymin + 35), fontType, 0.5, 255)
							cv.putText(depthFrameColor, f"Z: {int(depthData.spatialCoordinates.x)} mm", (xmin + 10, ymin + 50), fontType, 0.5, 255)
